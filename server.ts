...
serve({
  ...
  async fetch(req) {
    ...
    const isStatics = path.startsWith("/assets/") || path.match(/\.(js|mjs|wasm|css|png|jpg|svg|ico)$/);

    if (isStatics && !existsSync(join(DIST_DIR, path))) {
      return new Response("/* Not Found */", { 
        status: 404, 
        headers: { "Content-Type": path.endsWith(".wasm") ? "application/wasm" : "application/javascript" } 
      });
    }
    ...
    indexRes.headers.set("Cache-Control", "no-cache, no-store, must-revalidate");
    return indexRes;
  }
});
